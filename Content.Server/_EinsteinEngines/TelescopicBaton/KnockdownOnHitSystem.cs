...
public sealed class KnockdownOnHitSystem : EntitySystem
{
    ...
    private void OnMeleeHit(Entity<KnockdownOnHitComponent> entity, ref MeleeHitEvent args)
    {
        ...
        // Sandwich-HL: adjusted code to our base
        List<EntityUid> knockedDown = new(); // Goobstation
        foreach (var target in args.HitEntities.Where(e => !HasComp<BorgChassisComponent>(e) && _mobState.IsAlive(e)))
        {
            // Vanilla SS14 TryKnockdown (nimmt nur target, zeit und refresh)
            if (_stun.TryKnockdown(target, entity.Comp.Duration, entity.Comp.RefreshDuration))
            {
